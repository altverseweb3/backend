...
# ==============================================================================
# ANALYTICS SYSTEM
# Functions for handling analytics read queries.
# ==============================================================================


def analytics_get_leaderboard(payload):
    """
    ...
    try:
        ...
        print(
            f"DynamoDB Query Error in analytics_get_leaderboard: {e.response['Error']['Message']}"
        )
        return build_response(500, {"error": "Could not query leaderboard data"})
    except Exception as e:
        print(f"An unexpected error occurred in analytics_get_leaderboard: {str(e)}")
        return build_response(500, {"error": "An internal server error occurred"})


def analytics_get_user_leaderboard_entry(payload):
    """
    ...
    try:
        ...
        print(
            f"DynamoDB BatchGet Error in analytics_get_user_leaderboard_entry: {e.response['Error']['Message']}"
        )
        return build_response(500, {"error": "Could not query user rank data"})
    except Exception as e:
        print(
            f"An unexpected error occurred in analytics_get_leaderboard_entry: {str(e)}"
        )
        return build_response(500, {"error": "An internal server error occurred"})
...
def handle_analytics(event):
    ...
    try:
        ...
        if query_type == "leaderboard":
            return analytics_get_leaderboard(body)
        elif query_type == "user_leaderboard_entry":
            return analytics_get_user_leaderboard_entry(body)
        else:
...
def lambda_handler(event, context):
    ...
    return build_response(44, {"error": "Not found"})
