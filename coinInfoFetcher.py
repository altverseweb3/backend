...
class CoinAggregator:
    def __init__(self):
        ...
        if not self.alchemy_api_key:
            print("Warning: No ALCHEMY_API_KEY found in environment variables.")
        ...
        self.COINS_LIST_URL = 'https://api.coingecko.com/api/v3/coins/list?include_platform=true'
    ...
    def fetch_data_from_coin_gecko_with_retry(self, url, params=None, max_retries=5):
        ...
        for i in range(max_retries):
            try:
                ...
                # Success - wait standard time before next request to avoid rate limits
    ...
    def fetch_alchemy_metadata(self, chain_name, contract_address):
        ...
        # Use the same parameter structure as in the example handler
        payload = {
            "id": 1,
            "jsonrpc": "2.0",
        ...
        try:
            ...
            if response.status_code == 429:
                ...
                time.sleep(2)
                # Try the request again without recursion to avoid stack overflow
                response = requests.post(url, headers=headers, json=payload)
                if response.status_code == 429:
            ...
            # Success - wait to respect rate limits (20 calls per second)
            ...
            print(f"Error fetching Alchemy metadata for {chain_name}:{contract_address}: {e}")
            self.log_error("alchemy_metadata", f"{chain_name}:{contract_address}", str(e))
            return None
    ...
    def enrich_with_alchemy_metadata(self, chain_name, tokens):
        ...
        if not self.alchemy_api_key or chain_name not in self.alchemy_networks:
            print(f"Skipping Alchemy enrichment for {chain_name}: No API key or unsupported chain")
            return tokens
        ...
        for i, token in enumerate(tokens):
            ...
            # Check if token already has Alchemy metadata
            if token.get('alchemy_metadata'):
                print(f"Token {token.get('name', 'Unknown')} already has Alchemy metadata")
                enriched_count += 1
                continue
            ...
            if metadata:
                ...
                enriched_count += 1
                print(f"Enriched [{i+1}/{len(tokens)}] {token.get('name')} ({token.get('symbol')})")
            else:
                print(f"No metadata found for [{i+1}/{len(tokens)}] {token.get('name')} ({token.get('symbol')})")
                
        print(f"Summary: Enriched {enriched_count} tokens, skipped {skipped_count} tokens, failed {len(tokens) - enriched_count - skipped_count} tokens")
        return tokens
    ...
    def fetch_all_coins_by_market_cap(self):
        ...
        while True:
            ...
            # Optional: Add a checkpoint to save progress every 5 pages
            if page % 5 == 0:
                checkpoint_file = os.path.join(self.logs_dir, 'market_cap_checkpoint.json')
                # Save only essential data to keep file size manageable
                checkpoint_data = [{
                    'id': coin.get('id'),
    ...
    def log_failed_download(self, image_url, save_path, error):
        ...
        with open(log_file, 'a') as f:
            ...
            f.write(f"{timestamp} | {os.path.basename(save_path)} | {image_url} | {error}\n")
    ...
    def clean_removed_tokens(self, chain_name, current_ids):
        ...
        removed_ids = [coin_id for coin_id in previous_ids if coin_id not in current_ids]

        if removed_ids:
            print(f"Removing {len(removed_ids)} tokens no longer in the list for {chain_name}:")
            for removed_id in removed_ids:
                png_path = os.path.join(self.base_dir, chain_name, 'pngs', f"{removed_id}.png")
    ...
    def fetch_token_images(self, chain_name, coins):
        ...
        for coin in new_coins:
            if coin.get('image_url'):
                png_path = os.path.join(self.base_dir, chain_name, 'pngs', f"{coin['id']}.png")
                success = self.download_image(coin['image_url'], png_path)
                if not success:
                    failed_downloads.append(coin['id'])
        
        if failed_downloads:
            print(f"Failed to download {len(failed_downloads)} images: {', '.join(failed_downloads)}")
    ...
    def has_contract_on_platform(self, coin_id, coins_with_platforms, platform_name):
        ...
        for coin in coins_with_platforms:
            if coin['id'] == coin_id:
                # Check if the coin has a contract on this platform
                if coin.get('platforms', {}).get(platform_name):
                    return coin['platforms'][platform_name]
                # Also check if this is the native token for the chain
                elif coin_id == platform_name.replace('-', '_') or coin_id == platform_name:
                    return 'native'
        return None
    ...
    def process_chains(self):
        ...
        for coin in all_coins_by_market_cap:
            ...
            for chain_name, platform_name in self.chains.items():
                contract_address = self.has_contract_on_platform(coin_id, coins_with_platforms, platform_name)
                if contract_address:
...
def main():
    try:
        ...
        # Command line argument for specific chain if provided
        import sys
        if len(sys.argv) > 1:
            ...
            if chain_arg in aggregator.chains:
                ...
                print(f"Chain '{chain_arg}' not found. Available chains: {', '.join(aggregator.chains.keys())}")
                return
        else:
            # Process all chains
        ...
        print(f"Chains with Alchemy enrichment: {', '.join(aggregator.alchemy_networks.keys())}")
        ...
        try:
            # Use the same logs directory as defined in the class
